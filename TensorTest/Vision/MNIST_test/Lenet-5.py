...
def main():
    ...
    train(x_train,y_train,training_operation,x,y)
...
#위코드의 문제 python에서의 call by value / reference 메커니즘이 C/C++ 등의 타언어와 다르다.
#return 시에 logit 값이 다르기때문에 이를 return시 문제가 생길 수 있다.
#python은 call by value / call by reference / call by assignment가 존재하는데
#이는 해당 변수가 mutable 타입 인지 immutable타입인지에 따라 자동적으로 변환된다

# TensorFlow Session 문제.
